...
This section assumes that you prepared your cluster and your custom-values.yaml file, using guidance provided in the [Planning your HCL DX 9.5 container deployment using Helm](../deployment/preparation/overview.md) topic, and then installed your deployment using the instructions in the [Install](../deployment/helm_install_commands.md) topic.

-   **Overview of Helm Configuration Updates**

    Once an HCL Digital Experience Kubernetes 9.5 deployment is installed, it is possible to update its configuration directly using the standard Kubernetes or OpenShift commands \(for example, by updating values in the various config maps\). However, this is NOT the recommended approach. Some of the configuration parameters have interdependencies, as outlined in the [Planning section](../deployment/preparation/overview.md). These require knowledgeable management to make changes that are compatible with interdependency requirements. For example, if you change the context root for DX Core you also need to change the readiness and liveness probes.
