...
# Managing Web Content Syndicators and Subscribers using DXClient | HCL Digital
 Experience


The section provides information about using the DXClient process to automate the
 ...
 Subscribers, see [How to manage syndicators and subscribers | HCL Digital Experience](../panel_help/wcm_syndication.html "Syndication is used to transport data from one instance of HCL Web Content Manager to another.").


## Managing syndicators


The manage-syndicator command is used to enable or disable the
 syndicator using the provided input.



Command description

This command invokes the manage-syndicator tool inside the DXClient. It
...
This command shows the help document on the manage-syndicator command
 usage:
```
...
Use this attribute to specify the UUID of the syndicator
 instance.
```
-UUID <value>
```

Use true or false to enable or disable the
 syndicator.
```
-enable <value>
```
...
The options that are passed through the command line override the default
 values.
...
The manage-syndicator get-syndication-report command is used to
 fetch the failed reports of the syndicator.



Command description

This command invokes the `syndicator-faileditems` tool
...
This command shows the help document on the manage-syndicator
 get-syndication-report command
 usage:
```
...
All the above command options can also be configured inside the
 config.json configuration file of the DXClient
 tool, available in the <working-directory>/store
 directory of the DXClient installation. This file is read by
 default.Note: If you have installed DXClient
...
Note: If UUID of a syndicator is specified, then
 ...
 failure report for all syndicators.
...
The manage-subscriber command is used to enable or disable the
 subscriber using the provided input.



Command description

This command invokes the manage-subscriber tool inside the DXClient. It
...
Use this attribute to specify the UUID of the subscriber
 instance.
```
-UUID <value>
```

Use this attribute to specify the enable or disable the subscriber
...
The `create-syndication-relation` command is used to create the
 syndication relation between syndicator and subscriber in the DX server.



Command description

Use the `create-syndication-relation` to create
 syndication
 relation:
```
...
This command shows the help information for
 `create-syndication-relation` command
 usage:
```
...
The path to the contenthandler servlet on the Script Application
 server:
```
-contenthandlerPath <value>
```

Syndicator URL of target server, for example,
 http(s)://host:port/wps/wcm:
```
-syndicatorUrl <value>
```

Use this attribute to specify the new syndicator
 name:
```
-syndicatorName <value>
```

Use this attribute to specify the new subscriber
 name:
```
-subscriberName <value>
```

Use this attribute to specify the Credential Vault Name of source
 server:
```
-vaultSlotName <value>
```

 Whether the syndicator/subscriber pair is enabled on creation: isEnabled
 (default is
 true):
```
-isEnabled <value>
...
Whether the syndicator/subscriber pair is updateAfterCreation :
...
Use this attribute to specify the Libraries Name of target
 Server:
```
-webContentLibraries <value> 
```

Use this attribute to specify the Subscriber URL, for example,
 http(s)://host:port/wps/wcm:
```
-subscriberURL <value>
```

Use this attribute to specify the Syndicator/subscriber
 mode:
```
-mode <value>
```
...
Use this attribute to specify the path to the context root on the DX
 server (for example,
 /wps):
```
-dxContextRoot <value>
...
Use this attribute to specify the path to the Virtual portal
 Context:
```
...
On this page* [Managing syndicators](#syndicatorsandsubscribers__deploydxapp)
* [Manage-syndicator get-syndication-report](#syndicatorsandsubscribers__section_zfd_c1c_2qb)
* [Managing subscribers](#syndicatorsandsubscribers__section_cwj_dpf_dpb)
* [Create Syndication Relation](#syndicatorsandsubscribers__section_ndj_bcd_lrb)
