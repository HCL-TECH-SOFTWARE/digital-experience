...
This topic provides information about the deployment and undeployment of
 portlets.
...
The `deploy-portlet` command is used to deploy one or more new or
...
XMLAccess file
This xml file should contain the definition of the web application along
 ...
 (samples/DeployPortlet.xml) or in DX server located
 in the following
 directory: PortalServer\_root/doc/xml-samples/DeployPortlet.xml.


Portlet Application web archive file
This web archive .war file should contain the necessary
 portlet artifacts for deployment, as per the JSR 286 portlet standard. Refer
 to [Importing WAR files | HCL Digital Experience](../admin-system/adxmlref_import_war.html "To create new portlet applications, you need additional resources, the WAR files.")
...
This command shows the help document on the `deploy-portlet` command
 usage:
```
...
Use this attribute to specify the config wizard home (change to the appropriate route
 in the case of an OpenShift Kubernetes Environment, otherwise the value will
 typically be the same as the hostname) that is required for authenticating with the
 DXConnect
 application:
```
...
All of the above command options can also be configured inside the
 ...
 available in the dist/src/configuration directory of the
 DXClient installation. Command options passed through the command line will override
 values set in the config.json file.
...
The `undeploy-portlet` command is used to undeploy the portlets in the
 ...
 `store/outputFiles/portlets/backup/undeploy-portlet/`. In
 ...
 exported deployable portlet application XML file.

Command description

This command invokes the undeploy-portlet tool inside the DXClient. The
...
This file should contain the definition of the web application along with
...
Use this attribute to take the backup of portlet application before
 undeploying
 it:
```
-enableBackup <value>
...
Use this attribute to specify the config wizard home (route change only
 in case of Open Shift Kubernetes Environment, otherwise same as
...
All the above command options can also be configured in the config.json configuration
 file of the DXClient tool, available in the
...
On this page* [Deploy Portlets](#portlets__section_xzq_tyv_v4b)
* [Undeploy portlets](#portlets__section_xjb_2hg_w4b)
