...
|
|Subscriber is successful but syndicator is pending with no updates or failed items|Verify that the subscriber URL is correct. For example, the subscriber URL might be one of the following URLs:

-   http://myportalhostname.ibm.com:port/wps/wcm/connect?MOD=Synd, where myportalhostname is the name of your portal host
-   http://mywebserverhostname.ibm.com:port/wps/wcm/connect?MOD=Synd, where mywebserverhostname is the name of your web server host
...
2.  Click the **Edit Syndicator** icon for your idle syndicator.
3.  Verify that the subscriber URL begins with either your portal host name or your web server host name. If it does not, update the URL so that it matches the examples of the subscriber URLs.

|
|Syndication takes a long time to complete on z/OS® systems.|When you run syndication with a large amount of data, the z/OS database 4k buffer pool, 4k index buffer pool, and the 32k buffer pool need to be monitored and increased as necessary. Increase of these buffer pools make significant impact on syndication performance. For more information, see *Preparing DB2 for z/OS*.|
