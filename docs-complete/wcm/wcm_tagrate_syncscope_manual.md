...
1.  To manually run synchronization, run the `cp-sync` configuration task or submit an XML request to the portal by using the XML configuration interface.
    ...
    -   To run synchronization with a configuration task, run the following task from the [wp\_profile\_root](../reference/wpsdirstr.md#wp_profile_root)/ConfigEngine directory:
