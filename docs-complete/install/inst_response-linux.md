# Creating a response file for installing HCL Digital Experience in Linux
...
You can also use the console mode to generate a response file. Go to [Installing with the console for a Linux environment](inst_console-linux.md) for more information. After you review the summary information, enter G to generate a response file instead of I to install.
...
7.  On Install Packages: Select packages to install, select the following packages, and then click **Next**:
    ...
    -   **IBM WebSphere SDK Java Technology Edition**
        ...
        **Note:** The **IBM WebSphere SDK Java Technology Edition** option is required for a HCL Digital Experience installation even though it might be marked as "Optional" on the Select packages to install screen.
...
**Parent topic:**[Installing HCL Digital Experience in Linux using a response file](../install/inst_silent-linux.md)
