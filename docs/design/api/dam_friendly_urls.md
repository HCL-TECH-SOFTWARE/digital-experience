...
Note that you can only set friendly URLs for assets with a unique asset name in a collection. You must rename existing assets that have duplicate names in a collection so you can set friendly URLs. Uploading an asset with a file name already in a collection returns an error message that says you must update the file name and try uploading again. You can provide a unique name when you save the asset as new.
