...
4. **For a remote migration only**: Complete the following steps to update the ports for the deployment manager and on every node:
    ...
    13. Continue to run the deployment manager and the node agent of the node that is being updated. However, stop all other node agents before you continue to the next step.
        ...
        !!! important
        If you update the SOAP port of the target Deployment Manager, then you must update the **WPS_SOAP_PORT** value in the wpscript.sh file. The wpscript.sh file is in the [wp_profile_root/PortalServer/bin/](../../../../guide_me/glossary/wpsdirstr.md) and [PortalServer_root/bin](../../../../guide_me/glossary/wpsdirstr.md) directory for each node.
