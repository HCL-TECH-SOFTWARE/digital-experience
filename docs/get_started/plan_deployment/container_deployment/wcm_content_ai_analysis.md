...
1. ```OPENAI_MODEL```: Currently supported AI model is ```text-davinci-003```. Starting CF216, the AI model is switched to ```gpt-3.5-turbo``` due to OpenAI discontinuing support for ```text-davinci-003``` starting January 4, 2024. Starting CF221, the AI model is switched to ```gpt-4o``` as the newest and most capable model with up to date information. However, the model can be overridden by overriding this property. 
