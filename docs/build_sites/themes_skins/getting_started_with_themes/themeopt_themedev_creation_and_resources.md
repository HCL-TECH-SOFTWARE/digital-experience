...
-   **Static resources**
    ...
    Static theme resources include the theme.html file, layout templates, skins, JavaScript, CSS, and image files. The resources are stored by default within WebDAV, which is accessible with any WebDAV client. Learn more about [WebDAV client](../admin-system/webdav.md) here.

-   **Dynamic resources**

    Dynamic theme resources are stored in a WAR file. Only the default dynamic content spots for the theme, which are JSP files, and the Default.jsp file that renders the theme are stored in the dynamic theme resources. Because you can create and add new custom dynamic content spots without modifying the dynamic resources, you are unlikely to need to change this file. Learn more about [Creating custom dynamic content spots](themeopt_themedev_create_dynamic_content_spots.md#).
