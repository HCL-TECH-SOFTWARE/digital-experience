...
With direct access to a virtual portal setup on HCL Digital Experience (DX), exporting collects all the necessary files and imports them immediately.

Before you can export a Site Builder template, the target virtual portal must have HCL DX, HCL Web Content Manager (WCM), and Content Template Catalog installed on the virtual portal. Site Builder \(page and library\) must also be installed. You must be assigned administrator access to Site Builder on the target virtual portal and it must be running HCL Portal.
