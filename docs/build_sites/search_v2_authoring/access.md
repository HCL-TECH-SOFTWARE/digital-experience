...
- **Advance Search Filter** - The **Advance Search Filter** panel allows users to refine search results using attributes and filters. It includes a toggle to enable or disable the panel, a header with a close icon, and an **Attribute Selector** to choose from Description, Title, Type, or All attributes (default). Users can add filters through the **Add Search Filter** dropdown, which supports Author (requires typing at least three characters to search for users or groups), Status (Draft, Published, Archived), and Last Modified (preset ranges or custom start and end dates). Applied filters are displayed in the panel with options to remove them individually, while the **Apply** button refreshes results based on selected filters and the **Clear** button resets all filters. Multiple filters can be combined together, and results are shown only for items matching all applied conditions. A limitation is that searches by Type attribute require exact spelling and an exact match.
