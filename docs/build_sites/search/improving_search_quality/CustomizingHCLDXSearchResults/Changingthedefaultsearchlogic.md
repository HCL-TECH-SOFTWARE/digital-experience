...
2. Search for the search collection configuration file. There should be one configuration file for each collection, the name format is `collection_config_***.xml` where *** is some a sequence of generated alphanumeric numbers. The files should be in the directory /apps/search/config/.
