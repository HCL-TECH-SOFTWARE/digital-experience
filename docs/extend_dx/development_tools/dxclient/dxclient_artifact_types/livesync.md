...
- During the live sync process between the local and DX servers, if any files from the WCM library are deleted on the DX server, they will not be restored from the local system, and there will be no local indication of this deletion. If a local file is modified, a 'File not found' error will occur when the sync attempts to upload the change. Since live sync does not support bidirectional synchronization, the local files will not be pushed back to the server in either case. To resolve this, you must stop the sync, manually re-add the local file to the server, and repull the WCM library.
