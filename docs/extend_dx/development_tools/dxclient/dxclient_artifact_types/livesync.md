...
- During the live sync process between the local and DX servers, if files from the Themes are deleted simultaneously on the DX server, they will not be restored from the local system. Since live sync does not support bidirectional synchronization, any local updates to these deleted files will result in a 'File not found' error. To resolve this, you can push your local copies back to the theme, but this will override the existing files on the server. To proceed, stop the sync and push the files again. If a full replacement is not desired, stop the sync and manually re-add the files to the theme on the server.
...
- During the LiveSync process between the local and DX servers, if any files from the WCM library are deleted on the DX server by a concurrent user, they will not be restored from the local system, and there will be no local indication of this deletion. This happens because LiveSync is only a unidirectional process (local to server). If the same file is then modified by the user locally, it will result in a 'File not found' error. The local files can not be pushed back to the server in either case. To resolve this, you must stop the sync, manually add the local file to the server, and pull the WCM library.
