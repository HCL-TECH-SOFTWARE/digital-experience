...
1. Create Content Provider Profiles:
   ...
   Within **Profile Host** set **Unique profile title** as `Leap CPP` and enter **Host name or IP address** as `https://native-kube-alina-leap.team-q-dev.com:443` which is the newly created native kube instance containing Leap and **Save** the content provider profile
...
2. Create Web Dock Applications: 
   ...
   Add **Unique web dock application title** as `Leap`, within thw **Content provider profile** dropdown select the newly created profile `Leap CPP` and add the **Resource path** as `/apps` and **Save** the web dock application.
...
3. Adding the Web Doc Application created to the DX Page:
   ...
   2. click on the plus icon in left navigation panel to **Add page components and applications** to the newly created Child page. Within that click on **Applications** tab and search for the newly created web dock application i.e `Leap`.
   3. Click on **Add To Page** button.
   4. The HCL Leap Page will be visible withing the HCL Digital Experience page.