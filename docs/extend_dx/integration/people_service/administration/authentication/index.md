...
To set up People Service with OIDC for authentication, see [OIDC configuration](./oidc_configuration.md). Additionally, ensure that DX is configured with the same OIDC provider by referring to [Configuring OIDC for HCL Digital Experience](../../../../../deployment/manage/security/people/authentication/oidc/index.md).
