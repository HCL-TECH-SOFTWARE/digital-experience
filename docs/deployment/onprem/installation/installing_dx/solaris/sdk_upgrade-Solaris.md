...
Ensure that IBM® WebSphere® Application Server and HCL Digital Experience are installed with SDK version 7.0. Ensure that your Portal profile is created before you upgrade the SDK.

**Solaris restriction:** SDK version 7.1 is not available for Solaris. CF12 SDK Version 8.0 is
...
6.  Run the following command to list the available SDK versions:
    ...
    -   Solaris: ./managesdk.sh -listAvailable
7.  Run the following command to enable all existing profiles to use the new SDK version:
    ...
    **Note:** version\_number is the supported SDK version number that was listed when you ran the managesdk -listAvailable command.
    ...
    -   Solaris: ./managesdk.sh -enableProfileAll -sdkname version\_number -enableServers
8.  Run the following commands to make the new SDK version the new default:

    -   Solaris: ./managesdk.sh -setCommandDefault -sdkname version\_number
