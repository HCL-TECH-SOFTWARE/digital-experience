...
Unless you selected to install only the binary files, you can log in to the portal and verify that your installation was successful. Access HCL Digital Experience with the http://yourserver:yourport/wps/portal format.

If you are not sure what your port number is, use the list-server-ports command to determine the port number. Run the list-server-ports task to generate the HCL Portal and HCL Web Content Manager\_PortMatrix.txt file. For example: ConfigEngine.sh list-server-ports -DWasPassword=password.

Run the list-server-ports task to generate the server1\_PortMatrix.txt file. For example: ConfigEngine.sh list-server-ports -DWasPassword=password.
...
-   [Roadmaps for installation and deployment](rm_installation.md)
-   [Roadmaps for migration](rm_migration.md)
...
-   **Configuration Wizard**
    ...
    http://localhost:10200/hcl/wizard
