# HP: Uninstalling HCL Digital Experience
...
4.  Choose one of the following uninstallation commands to uninstall HCL HCL Digital Experience and WebSphere Application Server:
    ...
    -   Console mode: Run the ./imcl -c task from the install\_root/Installation Manager/eclipse/tools directory. Then, type 5.
        ...
        **Restriction:** You can select only one package group to uninstall at a time. First, uninstall the HCL HCL Digital Experience package. Then, uninstall the WebSphere Application Server package.

    During the uninstallation, remove all profiles that you do not want to keep for future installations.

5.  Remove any remaining HCL HCL Digital Experience directories from your directory structure.

6.  If you uninstall HCL HCL Digital Experience only, go to the [AppServer\_root](../reference/wpsdirstr.md#was_root) directory and remove the following files:
...
7.  Examine all running processes and stop ones that contain the PortalServer\_root directory. Restart the server, especially if you intend to reinstall HCL HCL Digital Experience on the same server.
...
10. After uninstalling HCL Digital Experience and WebSphere Application Server, you can also delete the Installation Manager. Run the uninstall.bat./uninstall command from the ibm/InstallationManager/uninstall directory.


**Parent topic:**[HP: Preparing to uninstall](../install/prep_uninst-HP.md)
