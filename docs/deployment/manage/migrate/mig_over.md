...
Migration is the process of collecting configuration data and applications from an earlier installed version of HCL Digital Experience (DX) and merging them into a newer installed version. This ensures the new environment is identical to the earlier environment. Migration to a different (remote) server is not supported.
...
HCL DX also supports integration with additional products to extend core functionality. If the earlier portal environment is configured to work with one or more supported products that provided integrated features, you need to follow the migration procedures for the integrated product.
...
You can migrate to HCL DX Version 8.5 or 9.5 from either Version 7.0 or 8.0 so long as you install CF13. For additional information about supported migrations, see *Supported migration paths*.

!!!note
    If you are migrating from HCL DX Version 8.0.0.1 on WebSphere® Application Server Version 8.5.5.2 to HCL DX Version 8.5/9.5, you must follow a different migration process. For more information, see *Migrating from Portal 8.0.0.1 on WebSphere Application Server 8.5.5.2*.
