...
To override the static user interface URL being set in React Integration Portlet, you can do it in WAS Server. In WAS Server, go to Resources and select Resources Environment Providers. In Resources Environment Providers, select WP_ConfigService and go to Custom properties to add new variables as provided below:
