...
In self-managed cluster deployments, the service account retains the imagePullSecret, which contains the license token created for authenticating image pulls from the ECR. However, the Runtime Controller Pod already employs its own service account to validate cluster resources. In our chart, we seamlessly integrate the imagePullSecret of the newly created service account, utilized for authentication from AWS Marketplace ECR, and patch it to the Runtime Controller Service account. This enables authentication when pulling the Runtime Controller image from the AWS Marketplace ECR. We achieve this by referencing the name of the imagePullSecret of the HCL service account used for AWS Marketplace.
