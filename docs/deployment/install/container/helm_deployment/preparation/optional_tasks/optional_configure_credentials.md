...
- Before upgrading to CF213 or higher, the `configWizardUser` and `configWizardPassword` under `security.core` must be updated to the current credentials of the **wpsadmin** user in the file-based user registry of Core **once**, followed by a Helm upgrade. After the Helm upgrade, Config Wizard credentials can now be updated if needed, separately from the Core. This is required because the file-based user registry of Core is copied to and reused by Config Wizard. For more information see: [Persisted Config Wizard Profile](../../../../../manage/portal_admin_tools/cfg_wizard/configuration/persist_cw_profile.md).
