...
- Before upgrading, temporarily set the DAM staging interval to 1440 (24 hours) using the dxclient manage-dam-staging trigger-staging command or adjust the cycleLength to 1440 via the DAM rest api at https://opensource.hcltechsw.com/experience-api-documentation/dam-api/#operation/StagingController.updateSubscriberData. Alternatively, you may disable DAM Staging while the publisher (source) and the subscriber (target) are not yet on the same CF version but be aware that reestablishing staging will restage all assets.
