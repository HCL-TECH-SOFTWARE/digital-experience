...
- Before starting the resync process, ensure that there are no failed staging operations. Use the above [OperationController.find API](https://opensource.hcltechsw.com/experience-api-documentation/dam-api/#operation/OperationController.find) to verify operations "FAILED" `sync` trigger functions. Currently, all failed operations are deleted after a [certain interval](../../operation_framework.md#failed-operations-clean-up-job).
