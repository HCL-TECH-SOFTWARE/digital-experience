...
-   The following are additional limitations at this time related to [DAM staging](../configuration/staging_dam/dam_subscription_staging.md):
    ...
    -   [Finding mismatch and Re-syncing DAM](../configuration/staging_dam/dam_staging_mismatch.md) items between publisher and subscriber can be achieved via DX-Client. There are certain limitations in terms of report generation and executing the resync. [Click here](../configuration/staging_dam/dam_staging_mismatch.md#limitations) to know more.
    -   DAM Staging between environments are supported only within the same CF versions.
