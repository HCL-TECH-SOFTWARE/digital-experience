...
The following are supported media asset file formats in HCL DX 9.5 Container. Starting with CF205, the WebP file type extension is also supported.
...
!!! note 
    ...
    - SVG file type is disabled by default. To enable the `.svg` file type (`image/svg+xml`) for asset uploads, use the `enabled` parameter in the [MediaTypeController.updateById](https://opensource.hcltechsw.com/experience-api-documentation/dam-api/#operation/MediaTypeController.updateById){target="_blank"} API.
