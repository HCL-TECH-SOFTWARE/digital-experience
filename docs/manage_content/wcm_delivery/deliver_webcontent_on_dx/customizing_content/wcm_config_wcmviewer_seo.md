...
3.  Create portlet preferences for each meta tag that you want to add to the page header.
    ...
    1.  Specify the portal preference for the name of the meta tag.
        ...
        !!! note
        Beginning with CF221, when the `meta.tag.name.suffix` value starts with `og:`, it's treated as the Open Graph protocol, and the attribute key is set to `property` instead of `name` by default.
