...
1.  Prepare the system using the steps outlined in [Configuring images for Document Conversion Services](./configuration/dcs_config_images.md).
...
IBM i: You can choose to run the X Server on the portal server or a remote server. You can also use a frame-buffer device or XVFB to emulate X Windows servers on a headless server. The conversion filters are installed with HCL Portal. There is no need for configuration. However, you can configure the export.cfg file, located under the [wp\_profile\_root](../../guide_me/wpsdirstr.md#wpprofileroot)/PortalServer/config directory, if you want to fine tune conversion properties such as graphics size and resolution.
