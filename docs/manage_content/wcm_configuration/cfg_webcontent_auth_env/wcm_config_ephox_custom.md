...
|`relative_urls`|`boolean`|`false`|For URLs with the same domain as the page containing the TinyMCE editor. If set to: <br>`true` — All URLs created in TinyMCE will be converted to a link relative to the base url. <br>`false` — All URLs will be converted to absolute URLs.|
