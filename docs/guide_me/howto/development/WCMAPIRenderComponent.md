...
The following lines of code demonstrate WCM API usage.

In the code, a `DocumentIDIterator` is defined that points to a Portal component (`PZNCMPT`). Then, a `libraryPath` and a `sitePath` are specified for use in the `RenderingContext`. Once the rendering context exists, the iterator iterates over all component IDs and prints the results.
