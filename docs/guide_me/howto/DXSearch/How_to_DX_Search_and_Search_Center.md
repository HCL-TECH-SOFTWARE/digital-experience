...
This article is written to get a better understanding for using the DX Search and the Search Center when trying to find different kind documents used in the HCL Digital Experience. It also provide detailed steps that can be used to prepare Web Content Management (WCM) articles to find them better with the search service (by defining metadata).  
