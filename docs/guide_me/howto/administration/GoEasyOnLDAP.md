...
3. If the membership attribute resolves nested groups, [disable nested groups in Portal.](https://help.hcl-software.com/digital-experience/9.5/sc-ls-test/deployment/manage/security/people/authorization/users_and_groups/adusrgrp_nested/).  
...
4. Configure a federated repository so that VMM resolves group membership during the WAS login process.  Then configure Portal to reuse WAS group information (user management only, to maximize performance).
    ...
    There are some additional tweaks you can make with PAC caches, VMM caches, and context pooling, but these four items should help a great deal in resolving any
    performance delays in resolving group membership during Portal login.
