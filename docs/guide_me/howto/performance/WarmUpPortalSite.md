...
1. Create either a .bat or .sh file depending on the OS platform on which the Portal server is deployed (Windows or Unix-based respectively).
...
4.  At this point, loading of Portal components in a browser will still be slow as the Portal server caches are not yet warmed up.  In the next part of the script, mimic access to the Portal server as an anonymouts user:
```shell
