...
If you run on a HCL Digital Experience version that is lower then Cumulative Fix (CF) version 221, you might see the problem. The log4j jars has been updated with CF221 to use version 2.23.1. With that the security scanning tools should not mark these log4j jars as a potential security leak anymore. 
...
3. Sequentially locate and delete (trashcan icon) each web module:
- unifiedtasklist.war
