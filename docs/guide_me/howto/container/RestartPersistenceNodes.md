...
2. Identify a node that will not be undergoing emergency maintenance and issue the command:
    ```text
    Kubernetes: kubectl -n dxns describe node xyz
    OpenShift: oc describe node xyz
...
4. Run the 'helm upgrade' command to apply the changes to the chart.
...
6. Run the 'helm upgrade' command to apply the changes to the chart.

7. Scale back up to your normal number of pods using the helm chart.

8. Apply Maintenance.

9. Once everything is stable again, remove the nodeSelector customizations from your helm chart and run the 'helm upgrade' command to once again distribute the workload across nodes.
