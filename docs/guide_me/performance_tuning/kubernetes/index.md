...
- Thread Groups: Concurrent users are simulated using Thread Groups and are configured using the Threads (Users), the Ramp-up Period, and the Loop Count.
    ...
    For example: Threads (Users): 100,  Ramp-up Period: 10 seconds, Loop Count: 1. This means JMeter will simulate loading 100 users over 10 seconds, leading to approximately 10 users per second.
...
- The concurrent user load distribution are as follows:
    ...
    Refer to the [DX Sizing rendering scenarios guide](./dxsizing_rendering_scenarios.pdf) for more information.
...
- There are 20 test portal pages labeled "PerformanceTest." Each page is accessible via a user-friendly URL format like `/perf/page-xx`.
...
- The tests used a total of eight unique pages with portlets for small configuration and 80 pages for medium and large configurations. 
