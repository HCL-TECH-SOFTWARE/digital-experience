...
This section describes the caches in HCL Portal 9.5 and provides best practices for their configuration. As highlighted in previous sections detailing performance benchmark modifications, cache size and lifetime are the most commonly adjusted properties when tuning Portal caches. You might consider increasing a cache's size if many values are used regularly and sufficient Java heap space is available. Similarly, you may extend the lifetime of cache entries if the cached data rarely changes and it's not critical for your business to reflect those changes immediately in your Portal. The configurations discussed in this section are typically set either in the Resource Environment Provider section of the WebSphere Integrated Solutions Console or within the `CacheManagersService.properties` file. For specific instructions on configuring caches, refer to the "Cache Manager Service" section in the base Portal tuning documentation.

Each cache description includes the following key attributes:

* **Default size, default lifetime, and cache usage pattern:** 
* **Cache content and scaling factor(s):** 
* **Read and write access information:** 

Some cache descriptions include a sample scenario with suggested property values.

## Access Control

his section details each of the access control caches. Maintaining current access control information is critical for proper Portal operation. Therefore, these caches must be shared within a cluster to ensure information propagates to all members. To prevent concurrent reloads from multiple caches, different lifetime values should be set. This approach of varied lifetime and invalidation intervals could also be applied to other caches.
...
This cache stores access control user context objects, serving as a local cache for permissions assigned to specific users. Whenever possible, access control requests are fulfilled using this information, allowing access control methods to return very quickly. This cache scales with the number of active users. For optimal Portal operation, you should ensure that entries for all actively working users fit into the cache, especially if a user has access to many Portal resources. Entries are invalidated from the cache upon any Portal administrative action. While creating a cache entry is typically inexpensive because most information is in memory, it can take longer if the required information isn't found in other caches. An entry in this cache can become very large, depending on the number of resources a user can access.
...
This cache contains the resource hierarchy within Portal access control. Its size scales with the number of protected resources accessed by active users in the system, similar to the protected resources cache. This cache does not contain leaf objects in the access control tree, so the number of entries is typically smaller. The cache is accessed during most Portal access control requests. Entries are invalidated from this cache during resource deletion, resource parent changes, modification of the resource owner, externalization, internalization, and role block change. Creating a cache entry includes a multi-row query against the Portal database. An entry in the cache is fairly small.
...
There is one entry per set of permissions per WebSphere Portal domain. Entries are read during regular access control requests, page rendering, and particularly during Portal administration. If a specific resource type is not used, you will observe only cache misses and no other activity on the corresponding cache. Entries are invalidated from this cache during all access control modifications and logins. Creating an entry in one of these caches can typically be done using in-memory information from lower-level caches. However, if the required information is not available, creating a cache entry may require multiple database requests. An entry in the cache is relatively small but is built from multiple objects typically stored in other caches.
...
This cache stores authorization information for pages that were defined outside a project and subsequently deleted as part of that project. Each entry contains the project identification and the database domain identification. Given that there are two possible domains (release or JCR), the maximum number of cache entries is twice the number of projects.
...
!!! Note 
    In Portal 8, this cache was combined into the `com.ibm.wps.ac.CommonProjectResourcesCache. com.ibm.wps.ac`.
 
**DataEventPropagationCache**
...
The protected resource caches contain the resources protected by Portal access control. The size of these caches scales with the number of protected resources accessed by the active users in the system. The `DependentProtectedResourcesCache` is used exclusively by WCM content item resources, while the `ProtectedResourcesCache` holds resources for various Portal resource types.

Entries are read from the cache during every permission or entitlement call against access control. Entries are invalidated from this cache during resource deletion, resource relocation, resource state modification (private/shared), resource owner modification, externalization, internalization, and role block changes. Creating a cache entry requires a single-row lookup in the Portal database. An entry in the cache is relatively small.
...
These caches contain additional access control data not directly cached in the `com.ibm.wps.ac.CommonExplicitEntitlementsCache`. They are dedicated to specialized resource types and uncommon use cases. Consequently, in a typical scenario, only some or none of these caches are filled, as the majority of the data is already cached in the `CommonExplicitEntitlementsCache`.
...
`NestedGroupCache` is used in a WebSphere Portal when nested groups are enabled. This cache contains the nested groups to which a user belongs. The size of this cache scales with the number of active users and the number of virtual Portals they access. The cache is accessed during Portal login, but typically not during regular Portal navigation. Its main use case is during the administration of users and user groups. Entries are invalidated from this cache after user and group administrative changes. Creating a new cache entry requires queries against the VMM component and, typically, then against the user repository. An entry in the cache is medium-sized.
...
This cache stores the mappings between pages and their derivation children, or empty mappings if no such children exist. Like the pageinstance.OIDCache cache, it is also accessed very frequently during page rendering and administration. Creating a cache entry involves one multi-row database query. This cache also scales with the number of pages in the system. Hence, you can use the same sizes for this cache as for `pageinstance.OIDCache` (assuming this is the intended comparison).
...
This cache stores information on Portal pages for fast retrieval during login or page navigation. It scales with the number of page instances in the system. It is one of the most frequently used caches and should be large enough to hold all pages that are frequently accessed by users.

Pages are loaded into the cache through direct navigation, by creating links to other pages, or by working with pages during Portal administration (always including all higher derivation levels). Creating a cache entry involves four multi-row database lookups. An entry in the cache is medium-sized.

To achieve best performance in terms of cache hit rate, the size should be set so that all pages defined in the system fit into the cache. This corresponds to the combined row count of the PAGE_INST database tables in the release, community, and customization databases.

**com.ibm.wps.datastore.pageinstance.OIDDraftCache**

**Default size: 500, default lifetime: infinite (-1), usage pattern: regular.**

his cache stores information on Portal draft pages for fast retrieval during login or page navigation. It scales with the number of draft page instances in the system. It should be large enough to hold all draft pages that are frequently accessed by users.

Draft pages are loaded into the cache through direct navigation, by creating links to other pages, or by working with pages during Portal administration (always including all higher derivation levels). Creating a cache entry involves four multi-row database lookups. An entry in the cache is medium-sized.

To maximize your cache hit rate and achieve the best performance, **size your cache to fully accommodate all draft pages**. This optimal size directly corresponds to the row count of the `PAGE_INST_DRAFT` table in your release database.
...
The model caches can be categorized into two groups:

1.  **Caches for assembled models as a whole** (for example, `com.ibm.wps.model.factory.UserSpecificModelCache`).
2.  **Caches for information within a model** (for example, caching a `ContentPage`).
...
his cache contains aggregated pages. Unlike the data store page instance cache, this cache holds the complete models of pages and their content (i.e., the portlets and containers on them). The page instance cache, conversely, stores only the raw page data.

This cache scales with the number of pages defined in your Portal and the different sets of access control rights on those pages. This cache contains very valuable information; it leverages several other caches (for example, page instance and access control caches) to build its data. Therefore, creating a cache entry usually only requires in-memory information, but it can also lead to many database queries.

The size of an entry in the cache depends on the complexity of the pages, but typically the objects are medium-sized, as they are usually made of references to other cached data. The cache should be large enough to hold the most frequently accessed pages multiplied by the number of different access control settings on these pages. Increasing the `cache lifetime` can be useful if page definitions do not change often in your environment.
...
his cache contains Portal topology information, i.e., Portal navigation elements composed of navigation nodes and their sorted, access-control-filtered children. Topology elements undergo several processing steps. First, they are loaded from the database, and eventually, they are added to the cache. This cache contains only the completely processed topology entities.

This cache is explicitly used during login and whenever a user navigates to a part of the Portal where they have not been before during the same session. If a cache entry is not found, a private copy is created and then further processed. Once the private copy is completely processed—which does not happen for all navigation nodes—it is added to the cache. If a user finds an entry in the cache, a reference is copied into their private topology model, and additional cache accesses are no longer necessary. Hence, there is only one cache hit (or miss) per user and navigation node.

The cache scales with the number of navigation nodes, the number of distinct permission sets on these nodes, and, possibly, the derivation chain (children and parents) to which a page belongs. Entries in this cache are expensive to create; they rely on other cached information, such as access control caches and the page instance cache. The entries in the cache are medium-sized, consisting mainly of references to other cached data. The cache should be sized to store the most important pages multiplied by all the different permission sets that exist on these pages.
...
n this cache, all models for all users are cached. In addition to user-dependent models, there are models such as the Content Model, which are session, markup, and device class-dependent.

Thus, the number of entries in this cache scales with the number of concurrently logged-in users on one cluster node, multiplied by the number of markups, and device classes. Additional entries are created if a user logs in more than once. A logged-in user with multiple sessions will cause more entries to be added to the cache. In addition, if many users are interacting with the Portal administrative pages, the number of entries will be doubled.
...
TThis cache stores policies. Out of the box, Portal comes with twelve theme policies and one mail policy, each constituting a single entry in the cache. Hence, the maximum number of cache entries depends on your system and the number of custom policies. This cache is accessed fairly often if policies are utilized. The HCL Portal 8.5 default theme uses policies and queries this cache during every request, but it is possible to create themes that do not use policies. Furthermore, when opening emails, the cache is accessed. Creating a cache entry involves reading data from a database. An entry in the cache is fairly small.
...
his cache stores connections between a policy and a policy target, for example, a user's distinguished name. Theme policies don't use targets, hence, there are no cache entries based on these policies. The out-of-the-box mail policy uses the user as its target. Hence, there's at least one entry for every user accessing the CPP mail portlet. The size of a cache entry depends on the size of the target object. For a distinguished name, a cache entry is fairly small.
...
his cache stores user-specific information. Each entry compiles a user's credential information across various LDAP directories and details which data for that user can be found in each directory. For example, general information might be stored in one directory, while their mail server and file path are in another.

The cache scales with the number of users working with Collaboration portlets. It's accessed whenever a Collaboration portlet, such as an HCL Sametime portlet, is accessed. Creating a cache entry can be fairly expensive since multiple resources might be queried. An entry in the cache is medium-sized.
...
D**efault size: 25, default lifetime: infinite (-1), usage pattern: regular**
...
This cache stores the configuration settings for the Outbound HTTP Connection Service. It is used whenever a component that utilizes this service, such as the AJAX proxy, is invoked. Cache entries are invalidated whenever changes to the outbound HTTP connections configuration are applied. To achieve optimal performance, the size of this cache should be greater than the total number of configuration settings for outbound HTTP connections, which include:
...
TThis cache stores the parent-child relations of configuration settings for the Outbound HTTP Connection Service. It is used whenever a component that utilizes this service, such as the AJAX proxy, is invoked. Cache entries are invalidated whenever changes to the outbound HTTP connections configuration are applied. To achieve optimal performance, the size of this cache should be greater than the total number of configuration settings for outbound HTTP connections.

In clustered environments, this cache must be shared, unless changes to the outbound HTTP connections configuration are not required on all cluster nodes.
...
This cache stores the mapping between Portal pages and WCM Portal Page artifacts that are used by the managed pages feature. It is primarily relevant on authoring systems where managed pages are enabled.

**com.ibm.wps.contentmapping.ContentMappingsCache**

**Default size: 1000, default lifetime: infinite (-1), usage pattern: regular**

This cache stores web content page resolution results. Web content page resolution means dynamically retrieving the right Portal page for rendering a given WCM content item or an IBM Connections resource. This resolution is typically performed when clicking on a link to a WCM content item (in the Web Content Viewer portlet, a Search result, or the Tag Center portlet) or a link to an IBM Connections resource in an IBM Connections portlet.
...
This cache contains content types in a specific format that is required by the portlet container. The objects held in this cache have another format and are processed differently from the objects in cache `com.ibm.wps.pe.contenttypes.nocharset`. This cache is used to avoid the calculation and formatting of response content types that are set by a portlet during a portlet invocation. The cache scales with the number of different response content types.
By default cache entries do not expire, this setting should not be changed. Rebuilding cache entries does not include database access but is computation intensive.
...
This cache contains an approximate number of personalized entities per shared portlet instance. It is used heuristically to avoid database access when determining the portlet instance that is associated with a portlet window. The cache scales with the number of shared portlet instances that are in active use in the system.

Increasing the default lifetime can improve performance if portlet instances change infrequently. Rebuilding the cache entries is relatively expensive, as it requires multiple database queries.
...
his cache contains portlet definitions. This cache holds objects of a different class compared to those in cache `com.ibm.wps.pe.portletmodel.portletdefinition`. The cache scales with the number of portlet definitions that are in active use in the system.

Increasing the default lifetime can improve performance if portlet definitions change infrequently.

Rebuilding cache entries is relatively expensive, as it involves loading data from the database through several calls.
...
In this cache, Java objects of type `javax.servlet.RequestDispatcher` are stored. The cache keys are pairs of `javax.servlet.ServletContext` and a `String` object representing the resource path.
...
The content of the cache can be viewed via `<http://portalserver/wps/mycontenthandler/distmap/ws/com.ibm.wps.spa.parser.skin.ThemeParserCache>`.<!--!link correct? does not seem to work?-->
...
This cache serves as a final lookup cache for the computed mappings between a hierarchy of URL mappings and a WebSphere Portal resource. It is accessed during every request to identify URL mappings from the incoming URL. The size of this cache should correspond to the total number of all mappings. Creating a cache entry is typically inexpensive because the information is often in-memory. An entry in the cache is relatively small.
...
This cache stores information about **tag instances**. A tag instance is a single tag associated with a single resource. For example, if a single resource `R1` has been tagged (by different users) with the tags `T1`, `T1`, and `T3`, three tag instances are created: one for `R1` tagged with `T1`, another for `R1` tagged again with `T1`, and a third for `R1` tagged with `T3`. If another resource `R2` has been tagged the same way (i.e., with the same tag names), three additional tag instances are created that hold information about how `R2` was tagged.

The cache scales with the **number of tag instances** in the system. It is accessed, and a new entry is added, whenever tag instances (distinct from tag spaces; see the `TagSpaceCache`) for a particular resource are queried. Thus, a reasonable size for this cache depends on the number of tags being assigned and queried.

Depending on the usage intensity of HCL Portal's tagging capabilities, a large number of tag instances can be quickly created. For instance, if a user invokes a custom tagging widget that can display all tag instances assigned to a particular resource, the cache might fill up quickly. It's advisable to ensure quick reloading of such a widget when invoking it for a resource that has been previously invoked. Thus, to achieve optimal performance in terms of cache hit rate, the size should be set so that all tag instances of a typical number of resources (for which tags are queried within the defined cache lifetime) fit into the cache.

---

It's also worth noting that the `TagCache` is usually less important and thus used less often than the `TagSpaceCache`. This is because users are often, for the same tag name, interested only in its count. The `TagSpaceCache` provides exactly this: information about how many times a particular tag (name) has been assigned to a particular resource. For example, for a particular resource `Rx`, it is usually only of interest how many times a tag (name) `T1`, `T2`, or `Tx` has been assigned; retrieving each single tag instance is usually less relevant.

**com.ibm.wps.cp.tagging.TagSpaceCache**

**Default size: 10000, default lifetime: 3600, usage pattern: regular.**

This cache stores information about **tag spaces**. A tag space is the aggregation of tags with the same name that have been associated with a single resource. For example, if a single resource `R1` has been tagged (by different users) with the tags `T1`, `T1`, and `T3`, two (not three, as with tag instances) tag spaces are created: one that holds the information that `R1` has been tagged with `T1` two times, and one that holds the information that `R1` has been tagged with `T3` once. If another resource `R2` has been tagged the same way (i.e., with the same tag names), two additional tag spaces are created that hold information about how `R2` was tagged.

The cache scales with the **number of tags** in the system. It is accessed, and a new entry is added, whenever tags for a particular resource are queried – in a way that tags with the same tag name can be aggregated as only their count is of interest. Thus, a reasonable size for this cache depends on the number of tags being assigned and queried. The standard tag widgets, for example, access this cache frequently.

Depending on the usage intensity of HCL Portal's tagging capabilities, a lot of tags can be quickly created. For instance, if a user invokes the standard tag widget (in order to query tags for a particular resource) in a short period of time and for a huge amount of different resources that all have been tagged a lot, the cache might fill up quickly. Again, it's advisable to ensure quick reloading of the standard tag widget when invoking it for a resource for which it has been invoked prior. Thus, to achieve optimal performance in terms of cache hit rate, the size should be set to a value so that all tag spaces of a typical number of resources (for which tags are queried within the defined cache lifetime) fit into the cache.
...
This cache stores information about **rating spaces**. A rating space is the aggregation of ratings of the same value that have been associated with a single resource. For example, if a single resource `R1` has been rated (by different users) with the values `5`, `5`, and `3`, two (not three, as with rating instances) rating spaces are created: one for `R1` rated two times with `5`, and another for `R1` rated once with `3`. If another resource `R2` has been rated the same way (i.e., with the same values), two additional rating spaces are created that hold information about how `R2` was rated.
...
To achieve best performance in terms of cache hit rate, the size should be set to a value so that all distinct tag names fit into the cache.

| Parameter | Value |
| :-------- | :---- |
| **Name** | `com.ibm.wps.cp.models.ModelCache.*` |
| **Models Included** | `TagModel`, `RatingModel`, `ResourceModel`, `CategoryModel` |
| **Default Size** | `200` |
...
## Cache Changes in HCL DX CF04

In HCL DX CF04, the following caches were removed, with the exception of `com.ibm.wps.services.vpmapping.VirtualPortalIDToRealmCache`. A new cache, `com.ibm.wps.services.vpmapping.VirtualPortalIDAndHostnameCache`, was added to support Virtual Portal (VP) to hostname mapping.
...
TThis cache maps virtual portal IDs to their respective LPID. The LPID is usually used to create URLs for a specific virtual portal. Since the number of LPIDs is equal to the number of virtual portal IDs, the optimum size of this cache is the number of virtual portals defined in your environment. You may increase the lifetime for better performance if your setup of virtual portals changes infrequently. If you only use the default portal and no additional virtual portal, you will see one entry in the cache and little cache traffic.
## WebDav
...
!!! Note
    All caches in the following includes the examples
...
The WSRP Consumer uses this cache to cache `getMarkup` responses. If Markup Caching is enabled for the Consumer or for a remote portlet, the Consumer checks this cache before sending a WSRP `getMarkup` request to the producer. It sends the request only if a corresponding markup response isn't found in the cache.

This cache is thus used to avoid sending WSRP `getMarkup` requests. It scales with the number of cacheable `getMarkup` responses received from the producer. You can adapt the cache size. The WSRP Consumer will always override the default lifetime according to the cache expiration provided by the remote portlet.
...
ese caches are WAS internal caches whose size is not expected to be modified by customers. When viewing these caches with the Portal Cache Viewer, the following text is displayed:
...
1. In the WebSphere Integrated Solutions Console, navigate to: **Security > Global Security > Custom properties > New** 

2. Create both of these security custom properties:
...
This cache is used in a cluster for Portals to notify other cluster members when one of the registries needs to be reloaded due to an administrative action. It should **never be disabled** or set to `shared=false`.
