...
his cache stores parent-child relationships that comprise a WCM navigator. A complex navigator might have multiple parent-child relationships (e.g., if siblings are included). The navigator entry consists of the IDs of the parent and children. This cache will be cleared upon any WCM Item update in the system.
...
This cache contains a lookup for library id, name and path to the library object. This is pre-populated up to the cache size at Portal ``startup.xml` format
...
This cache operates using a Least Recently Used algorithm. It isn't shared across nodes in the cluster, and it doesn't use DynaCache. It also doesn't update when LDAP changes. This cache is disabled by default but can be enabled by setting: `user.cache.enabled=true`.

### How to Set

This is set in `WCMConfigService.properties`. To enable it, you need to either run a module called `MemberCacheManager` or restart the server.

To enable the module, add the following properties to `WCMConfigService.properties`:
