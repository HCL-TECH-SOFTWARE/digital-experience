...
1.  **Caches that scale with the number of active users.**
    For example, the access control user context cache: `com.ibm.wps.ac.AccessControlUserContextCache`.
    
2.  **Caches that scale with the number of users using a specific function.**
    For example, the cache: `com.lotus.cs.services.directory.ldap.BasicLDAPDirectoryService.user`.
    
3.  **Caches that scale with the number of pages being visited.**
    For example, the resource cache: `com.ibm.wps.model.content.impl.ResourceCache`.

4.  **Caches that scale based on the growth of some other resource, such as clients or user agents**, which are stored in the cache.
    For example: `com.ibm.wps.model.impl.RuntimeClientMap.userAgent2client`.
