...
|Maximum Size <br> A value of 0 allows the pool to grow as large as needed.
If access to the LDAP server is shared by many systems, this setting may allow an excessive number of
connections to the LDAP server; in such a case, set the maximum pool size to a value appropriate to
your environment.|20 |40|
...
If your LDAP supports the group membership attribute, it is recommended to configure VMM to use it to gain a performance benefit. Details on the use of this attribute and steps to configure VMM to use it can be found here:
https://help.hcltechsw.com/digital-experience/8.5/plan/plan_vmm_int.html?query=vmm