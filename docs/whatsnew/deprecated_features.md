...
|--------|-----------|------------------|
|Configuring |OpenShift Passthrough| To deploy OpenShift manually using `Routes` you need to create a .yaml file and add the changes required, after which those chnages can be applied by running `kubectl apply` and specifing the namespace and location.<br>Documentation resource: [Create the route resource manually](https://opensource.hcltechsw.com/digital-experience/CF210/deployment/install/container/helm_deployment/preparation/mandatory_tasks/prepare_configure_networking/?h=create+route+resource+manuall#openshift-passthrough)|
