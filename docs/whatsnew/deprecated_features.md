...
|--------|----------|------------------|--------------|
|Deploying | Automated Pod restart on ConfigMap updates | Beginning with HCL Digital Experience 9.5 Container Update CF221, Pods will not be restarted automatically anymore by Runtime Controller when a ConfigMap is changed manually. For production deployments always use the Helm custom values and `helm upgrade` to change configuration. This will trigger the appropriate restarts. For development and testing when a ConfigMap is changed, the appropriate Pod will have to be restarted manually | June 2024 | 
