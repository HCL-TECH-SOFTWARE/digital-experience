...
=== "Containers"
    Starting from CF233, to enable DAM Indexing, you need to set the `damIndexing` parameter to `true` in your `values.yaml` file and ensure all required `searchMiddleware` configurations are set. For more information, refer to [DAM Indexing](../../manage_content/digital_assets/configuration/dam_indexing/configure_dam_indexing.md).
